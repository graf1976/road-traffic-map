...
| 道路名の表示 | 地図上の道路にカーソルを合わせる（スマートフォンはタップ）と、道路名と管理会社（NEXCO各社など）の交通情報へのリンクを表示 |
| 公式サイトへの案内 | 表示中の地方（関東・近畿など）に応じて、道路管理者の公式サイトへのリンクを表示 |
| 種別フィルタ | 通行止め／工事中／車線規制／チェーン規制で絞り込み。地図と一覧が連動 |
| 現在地取得 | 地図右上の現在地ボタンから現在地へ移動＋ピンを表示 |
| 自動更新 | 自動更新なし／1 分／5 分／10 分 から選択。次回更新までの残り秒数も表示 |
| 凡例 | 地図上に色の意味を表示（通行止め・規制の黒線は、実際に描画しているときだけ載せる） |
| エラー表示 | 位置情報の失敗・API 取得失敗・API キー未設定・描画エラーをそれぞれ画面上で案内 |
| ホーム画面に追加 | Web アプリマニフェストとサービスワーカーを備え、ブラウザのメニューからホーム画面に追加できる |
...
│   ├── api/road/route.ts          指した地点の道路名と管理会社を返す API
...
このアプリは規制情報を独自に転載していません。提供元を設定していない場合でも、
画面下部に**表示中の地方に応じた公式サイトへのリンク**を出し、正確な情報は
道路管理者のサイトで確認してもらう方針です（地図を動かすとリンク先も変わります）。

### 見本データ（開発用）

動作確認のために、実在する道路に沿った見本データ 8 件を同梱しています。
線形は OpenStreetMap から生成しているため、表示時は「線形 © OpenStreetMap contributors」を出します。
...
地図で指した地点の道路名は、OpenStreetMap をもとにした経路探索サービス（OSRM）の
`nearest` に問い合わせて求めています（[`src/app/api/road/route.ts`](src/app/api/road/route.ts)）。

- 画面の縮尺に応じて「その道路を指した」とみなす距離を変える（8px 相当、30〜400m）
- いちばん近い区間は名前の無いランプのことがあるため、候補を 5 件見て最初に名前のあるものを採る
- 約 10m 四方＋ズーム単位で 10 分間キャッシュし、問い合わせを減らす
- 取得できないときは何も表示しない（エラーにしない）
- 道路名のすぐ下に「道路名 © OpenStreetMap contributors」を表示し、
  [ライセンス（ODbL）の案内](https://www.openstreetmap.org/copyright)へリンクする

**制約**: 高架の都市高速（首都高など）の下に一般道がある場所では、真下の一般道の名前が
出ることがあります。位置だけでは上下の道を区別できないためです。
公開サービスの利用条件上、大量アクセスには向きません。本格運用する場合は
Google の Geocoding API（有料）や OSRM の自前運用に差し替えてください。
...
- `official-links` … 緯度経度からの地方判定と、公式サイトのリンク定義
